...
- **Payments are not wired.** The `PaymentProvider` interface, plans, subscriptions and unlock records all exist, but bKash and SSLCommerz need merchant credentials. Until then no purchase can complete, so every account behaves as Free.
...
- **The two independent security verifiers did not finish their run.** All 11 findings from the adversarial review were fixed and the fixes were manually spot-checked, but they have not been re-attacked end to end. Treat the security posture as good-but-unverified until that pass completes.
