...
export async function readComparison(journeyId: string, userId: string | null) {
  const journey = await db.journey.findFirst({
    ...
    select: {
      ...
      budgetMaxBdt: true,
      status: true,
      // The FULL item shape rather than a summary, because the traveller's own
      // comparison screen renders these through `toItemView` — the same
      // projection their workspace uses. A thinner select here would have meant
      // two shapes for one item and a second set of components to render it.
      items: {
        orderBy: [{ dayNumber: 'asc' }, { sortOrder: 'asc' }],
        select: {
          id: true,
  ...
  return {
    journey: {
      ...
      partyAdults: journey.partyAdults,
      partyChildren: journey.partyChildren,
